...
import { X, Package, Tag, DollarSign, Hash, Layers, Plus, Camera, Trash2 } from 'lucide-react'
import { addProductImage, createProduct, createCategory } from '@/lib/actions'
...
export function ProductModal({ categories, defaultMinStock, existingProducts, onClose, onSuccess }: Props) {
  ...
  const handleImageFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      setError(currentError instanceof Error ? currentError.message : 'Não foi possível processar as imagens selecionadas.')
    }
  }
    ...
    startTransition(async () => {
      try {
        ...
        if (pendingImages.length > 0) {
          await Promise.all(pendingImages.map((image) => addProductImage(createdProduct.id, image.src)))
        }
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 rounded-xl border border-border bg-muted/20 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Selecione várias imagens para a vitrine pública.</p>
              </div>
                ...
                {pendingImages.map((image) => (
                  <div key={image.id} className="relative overflow-hidden rounded-lg border border-border bg-background">
                    <div className="relative aspect-square">
                      <Image src={image.src} alt={image.name} fill className="object-cover" unoptimized />
                    </div>
                    <button
                      type="button"
          ...
          <label className="flex items-center gap-3 rounded-lg border border-border bg-muted/20 px-4 py-3 text-sm">
            ...
            <span>
              ...
              <span className="block text-xs text-muted-foreground">Quando desmarcado, o produto continua no SaaS, mas não aparece na vitrine pública.</span>
            </span>
          </label>
