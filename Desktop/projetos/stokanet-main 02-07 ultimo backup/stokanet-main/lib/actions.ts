...
export async function getProducts(search?: string, status?: string, includeArchived = false) {
  try {
    ...
    return prisma.product.findMany({
      ...
      include: { category: true, images: { select: { id: true, imageUrl: true, displayOrder: true }, orderBy: { displayOrder: 'asc' } } },
...
export async function createProduct(data: {
  ...
  const createdProduct = await prisma.product.create({
    data: {
      ...
      storePublished: data.storePublished ?? true,
...
export async function updateProduct(id: string, data: {
  ...
  await prisma.product.update({
    ...
    data: {
      ...
      highlights: data.highlights === undefined ? undefined : data.highlights?.trim() || null,
      ...
      storePublished: data.storePublished ?? product.storePublished,
      status,
    },
  })
  revalidatePath('/estoque')
...
export async function addProductImage(productId: string, imageUrl: string) {
  ...
  const maxDisplayOrder = await prisma.productImage.aggregate({
    where: { productId },
    _max: { displayOrder: true },
  })

  const newDisplayOrder = (maxDisplayOrder._max.displayOrder ?? -1) + 1
...
export async function removeProductImage(imageId: string) {
  ...
  const product = await prisma.product.findFirst({
    where: { id: image.productId, companyId },
    select: { id: true },
  })
  if (!product) throw new Error('Produto não encontrado')
...
export async function savePendingSale(data: {
  ...
  if (!data.items?.length) {
    throw new Error('Adicione ao menos um item para salvar a venda pendente.')
  }
  ...
  if (sanitizedItems.length === 0) {
    throw new Error('Itens inválidos para venda pendente.')
  }
  ...
  const resolvedItems = sanitizedItems.map((item) => {
    ...
    if (!product) {
      throw new Error('Produto inválido na venda pendente.')
    }
  ...
  const discount = Number.isFinite(Number(data.discount)) ? Math.max(0, Number(data.discount)) : 0
  const boundedDiscount = Math.min(discount, subtotal)
  const total = Number(Math.max(0, subtotal - boundedDiscount).toFixed(2))
  const saleCode = `VP-${Date.now().toString().slice(-8)}`
  ...
  await logAudit({
    ...
    entity: 'SALE',
    entityId: sale.id,
    details: `Venda ${sale.code} salva como pendente`,
    companyId,
    userId: user.id,
  })
...
export async function finalizePendingSale(input: {
  saleId: string
  ...
  if (!saleId) {
    throw new Error('Venda inválida para finalização.')
  }
  ...
  if (!isPendingSale(sale.saleStatus)) {
    throw new Error('Apenas vendas pendentes podem ser finalizadas por aqui.')
  }
  ...
  if (!sale.items.length) {
    throw new Error('A venda pendente não possui itens para finalizar.')
  }
  ...
  if (products.length !== productIds.length) {
    throw new Error('Um ou mais produtos da venda pendente não foram encontrados.')
  }
  ...
  await prisma.$transaction(async (tx) => {
    for (const item of sale.items) {
      const product = productMap.get(item.productId)
      if (!product) {
        throw new Error('Produto da venda pendente não encontrado.')
      }
    ...
    await tx.movement.createMany({
      data: sale.items.map((item) => ({
        type: 'SAIDA' as const,
        quantity: item.quantity,
        reason: `Finalização da venda pendente ${sale.code}`,
        productId: item.productId,
        companyId,
      })),
    })
    ...
    await tx.sale.update({
      ...
      data: {
        ...
        paymentMethod: input.paymentMethod?.trim() || sale.paymentMethod,
        notes: input.notes?.trim() || sale.notes,
  ...
  await logAudit({
    ...
    entity: 'SALE',
    entityId: sale.id,
    details: `Venda ${sale.code} finalizada a partir de pendente`,
    companyId,
    userId: user.id,
  })
...
function isPendingSale(saleStatus: string | null | undefined) {
  return (saleStatus ?? '').trim().toUpperCase() === 'PENDENTE'
}
...
export async function updateSale(input: {
  ...
  const saleIsPending = isPendingSale(sale.saleStatus)
  ...
  await prisma.$transaction(async (tx) => {
    if (!saleIsPending && stockDeltas.size > 0) {
