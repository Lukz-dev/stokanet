...
export default async function proxy(req: NextRequest) {
  ...
  const isPublicStoreApiRoute = pathname.startsWith('/api/loja/')

  if (isPublicStoreRoute || isPublicStoreApiRoute) {
    return NextResponse.next()
  }
...
export const config = {
  matcher: [
    '/((?!api/auth|api/loja|login|signup|_next/static|_next/image|favicon.ico).*)',
  ],
}
